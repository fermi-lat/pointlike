/** @file UbPointSourceLikelihood.h

$Header: /nfs/slac/g/glast/ground/cvs/pointlike/pointlike/UbPointSourceLikelihood.h,v 1.3 2007/07/14 03:50:54 burnett Exp $
*/

#ifndef tools_UbPointSourceLikelihood_h
#define tools_UbPointSourceLikelihood_h
...
namespace pointlike {
    ...
    /** @class UbPointSourceLikelihood
    @brief manage a set of UbSimpleLikelihood objects, one for each energy band

    Note that it is a map of the UbSimpleLikelihood objects, with the key the Healpix level,
    usually starting at 6, for 0.9 degree bins.


    */
    class UbPointSourceLikelihood : public  std::map<int, UbSimpleLikelihood*> {
    public:
        /** ctor
        ...
        */
        UbPointSourceLikelihood(const std::vector<astro::Photon> m_data_vec,
            std::string name,
            const astro::SkyDir& dir,  
            double radius=5.0,
            int minlevel=6, int maxlevel=13);

        ~UbPointSourceLikelihood();

        std::vector<astro::Photon> extract(const std::vector<astro::Photon> data,int level);
        ...
        static void setDefaultUmax(double umax){ UbSimpleLikelihood::s_defaultUmax=umax; }
