"""
Utilities for managing and plotting Healpix arrays

classes:
    HParray : base class for wrappers around HEALPix arrays
    HEALPIXFITS(list) : 
...
[Copied here, to uw/utilities from uw/like2/pub on 10/28/2018]
"""
import os,glob,pickle, types, copy, zipfile, healpy
import pylab as plt
...
class HParray(object):
    """ base class, implement a HEALPix array, provide AIT plot
        callable with SkyDir, or (l,b) tuple if nside is a power of 2 
    ...
    def __init__(self, name, vec, sigma=None, unit=None): 
        ...
        self.power_of_2 = np.log2(self.nside) % 1 ==0
    ...
    def __repr__(self):
       return '{}({!r}, nside={!r})'.format(
           self.__class__.__name__,
           self.name, self.nside)
    ...
    def __call__(self, skydir):
        """
        skydir: SkyDir instance | (l,b) tuple
        Return value at pixel if skydir is a SkyDIr instance
            otherwise expect (l,b) tuple, and use healpy to interpolate
            (not valid unless nside is power of 2)
        """
        if isinstance(skydir, SkyDir):
            return self[self._indexfun(skydir)]
        else:
            return healpy.get_interp_val(self.vec, *skydir, lonlat=True) 

    def get_interp_val(self, glat, glon ):
        """ return interpolated value(s)
        """ 
        return healpy.get_interp_val(self.vec, glat, glon, lonlat=True)
    ...
    def plot_along_plane(self, glat=0, ax=None, ylabel='Counts/pixel', label=None,**kwargs):
        glon = np.linspace(-180, 180, 1000)
 
        y = self.get_interp_val( glon, glat)
        fig, ax = plt.subplots(figsize=(8,4)) if ax is None else (ax.figure, ax)
        ax.plot(glon, y, '-', label=self.name if label is None else label)
        ax.grid(alpha=0.5)
        ax.set_xticks(np.linspace(-180,180,13))
        ax.set(xlim=(180,-180), xlabel='$l$', 
               title='{} along the plane'.format(self.name), ylabel=ylabel,**kwargs)
        
    def plot_along_meridian(self,glon=0, ax=None, ylabel='Counts/pixel' ,label=None, **kwargs):
        glat = np.linspace(-90, 90, 1000)
        y = self.get_interp_val( glon, glat)
        fig, ax = plt.subplots(figsize=(8,4)) if ax is None else (ax.figure, ax)
        ax.plot(glat, y, '-', label=self.name if label is None else label)
        ax.grid(alpha=0.5)
        ax.set_xticks(np.linspace(-90,90,13))
        ax.set(xlim=(-90,90), xlabel='$b$',                
               title='{} along the meridian'.format(self.name), ylabel=ylabel,
              **kwargs) 

    def plot_ZEA(self, center, size, pixelsize=0.1, title=None, axes=None, fignum=31, 
        zea_kw={}, galactic=True, cbtext=None, **kwargs ):
        """ center : tuple
                RA,DEc or L,B depending on galactic
            size : float
            cbtext : None | text
                if not None, draw colorbar with text
        """
        ...
        if cbtext is not None:
            zea.colorbar(label=cbtext)
        return zea
    ...
    def smoothing(self, sigma=2.5):
        """ gaussian smoothing
        nside must be power of 2

        sigma: float in degrees
        """
        assert self.power_of_2, 'Require nside to be power of 2'
        self.vec = healpy.smoothing(self.vec, sigma=np.radians(sigma))
...
def multi_ait(hparrays, vmin=None, vmax=None, log=None, cmap=None, grid_color=None, 
         title=None, nx=2, **kwargs):
    """Make a grid of AIT plots, all with same scaling and common colorbar

        hparrays: list of HParray objects (or a single one)
        vmin, vmax, log, cmap : parameters for the HParray.plot function
        grid_color : None | string
            if not None, apply a grid with given color
        clabel: string for colorbar label (or cbtext)
        nx : number of columns to use
        title : used for suptitle

    """
    if not hasattr(hparrays, '__iter__'): 
        hparrays=[hparrays]
        nx=1
    n = len(hparrays)
    ny = (n+nx-1)//nx
    cbtext = kwargs.get('cbtext', kwargs.get('cblabel', '')) 
    fig, axx = plt.subplots(ny,nx, figsize=(15,3.2*ny+3.0), 
            gridspec_kw={'wspace':0.05, 'hspace':0.05,'left':0.0, 'top':0.91})
    axx = axx.flatten() if n>1 else [axx]
    kw = dict(vmin=vmin,vmax=vmax, cmap=cmap, log=log, ait_kw=dict(nocolorbar=(n>1),cbtext=cbtext))
    
    for i,hp in enumerate(hparrays):
        ax = axx[i]
        q = hp.plot(axes=ax, **kw )
        if grid_color: q.grid(color=grid_color)
    for ax in axx[n:]: ax.set_visible(False)     
    #put colorbar at right  
    if n==1: return      
    cbax = fig.add_axes((0.92 if n>1 else 0.75, 0.15, 0.01, 0.6) )
    cb=plt.colorbar(q.mappable, cbax, orientation='vertical')
    cb.set_label(cbtext)
...
### needs testing--may not need anyway
# def expand(a, n=2):
#     """ expand the nside for a HEALPix RING array by factor n, which must be a power of 2"""
#     alen=len(a)
#     anside= int(np.sqrt(alen/12.))
#     bnside = anside * n
#     anest= a[healpy.nest2ring(anside, range(alen))].reshape(alen,1)
#     bnest = np.zeros((alen, n**2)) 
#     bnest+= anest
#     blen= alen * n**2
#     b= bnest.reshape(blen)
#     return b[healpy.ring2nest(bnside, range(blen))]
...
class HEALPixFITS(list):
    """Create FITS-format tables from multiple HParray objects 
    """
    ...
    def __init__(self, cols, nside=None, energies=None):
        ...
        if energies is not None:
            assert len(energies)==len(cols), 'Number of energies and columns must match'
    ...
    def make_hdus(self):
        """Return a FITS HDU list, with optional table of energies
        """
        ...
        if self.energies is not None:
            hdus.append(
                pyfits.BinTableHDU.from_columns(
                    [pyfits.Column(name='energies', format='D',
