...
class DiffuseFits(roi_info.ROIinfo):
    ...
    def correction_plots(self, cc, vmin=0.5, vmax=1.5, title='Galactic fits', hist=False):
        if isinstance(cc, pd.DataFrame):
            cc = cc.as_matrix()
        assert cc.shape==(1728,8), 'Found shape {}'.format(cc.shape)
        if hist:
            hkw=dict(bins=np.linspace(vmin,vmax, 21), lw=2, histtype='step')
            fig,axx = plt.subplots(2,4, figsize=(12,6), sharex=True, sharey=False)
        ...
        for i,ax in enumerate(axx.flatten()):
            if hist:
                ax.hist(cc[:,i].clip(vmin, vmax), **hkw)
