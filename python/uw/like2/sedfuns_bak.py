...
class SED(tools.WithMixin):
    ...
    """
    def __init__(self, rstat, source_name, quiet=False, tol=0.2):
        ...
        self.energies = sorted(list(set(np.array([b.band.energy for b in self.rs if b.pixels>0]))))  
    ...
    def select(self, index, event_type=None, poisson_tolerance=None, 
        ...
        """
        if poisson_tolerance is None:poisson_tolerance=self.tolerance
    ...
    def all_poiss(self, event_type=None, tol=None, debug=False):
        ...
        for i,e in enumerate(self.energies):
            if debug: 
                print '%3i %8.0f' % (i,e),
                pf = self.select(i, event_type=event_type,poisson_tolerance=tol, debug=debug)
                print pf
                pp.append(pf.poiss)
            else:
                try:
    ...
    def sed_rec(self, event_type=None, tol=0.2):
        ...
        #for i,energy in enumerate(self.energies):
            ...
            try:
                pf = self.select(None, elow=elow,ehigh=ehigh, event_type=event_type, poisson_tolerance=tol)
    ...
    def data_frame(self, event_type=None, tol=0.2):
...
def sed_table(roi, source_name=None, event_type=None, tol=0.2):
    """
    Return a DataFrame
    """
