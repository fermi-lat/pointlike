...
$Header: /nfs/slac/g/glast/ground/cvs/pointlike/python/uw/like2/pipeline/check_converge.py,v 1.24 2013/12/22 03:15:49 burnett Exp $
"""
...
def main(args):
    ...
    def make_zip(fname,  ext='pickle', select=None):
        ...
        if len(ff)!=1728 and ext=='pickle':
            raise Exception('Found %d pickle files, expected 1728')
    ...
    def create_stream(newstage, other=''):
        cmd = '/afs/slac/g/glast/ground/bin/pipeline  createStream -D "stage=%s, POINTLIKE_DIR=%s, SCRIPTS=%S, SKYMODEL_SUBDIR=%s %s" UWpipeline '\
            %( newstage, pointlike_dir, pointlike_dir, skymodel, other)
        if args.test:
        ...
        if stage!='update_only':
            if max(r.niter)<8 and len(q)>1:
                if len(qq)> 200:
                    ...
                    create_stream('update', other=', job_list=update_roi_list.txt')
            else:
