...
class HPtables(Diagnostics):
    ...
    def ts_map(self, vmin=10, vmax=25, pixelsize=0.25):
        """ TS residual map 
        DIstribution of TS values for residual TS study.
        """
        hpts = healpix_map.HParray('ts', self.tables.ts)
        hpts.plot(ait_kw=dict(pixelsize=pixelsize), vmin=vmin, vmax=vmax)
        return plt.gcf()
        
    def seed_plots(self, seedfile='seeds.txt'):
        """ Seed plots
        
        Results of cluster analysis of the residual TS distribution. Read from file %(seedfile)s. 
        ...
        """
        if not os.path.exists(seedfile) or os.path.getmtime(seedfile)<os.path.getmtime(self.fname):
            print 'reconstructing seeds: %s --> %s' % (self.fname, seedfile)
            os.system('python -m uw.like2.pipeline.check_ts %s %s' % (self.fname, seedfile))

        self.seedfile=seedfile
        z=pd.read_table(seedfile)
        ...
        plt.setp(ax, xlabel='size'); ax.grid()
        ax=axx[2]
