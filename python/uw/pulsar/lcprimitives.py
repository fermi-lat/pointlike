"""
A module containing LCPrimitive and its subclasses.  They implement components 
of a pulsar light curve.  Included a primitives (Gaussian, Lorentzian), etc. 
    ...
    def __str__(self):
        ...
        try:
            ...
            for i in xrange(len(self.pnames)):
                n=self.pnames[i][:m]
                t_n = n+(m-len(n))*' '
                l += [t_n + ': %.4f +\- %.4f'%(self.p[i],self.errors[i])]
        except:
            for i in xrange(len(self.pnames)):
                n=self.pnames[i][:m]
                t_n = n+(m-len(n))*' '
...
class LCLorentzian(LCWrappedFunction):
    ...
    def init(self):
        ...
        self.free = np.asarray([True,True,True])
        self.pnames = ['Norm','Width','Location']
...
class LCLorentzian2(LCPrimitive):
   """Represent a two-tailed Lorentzian peak.  Note the assumption made on how to construct
      the cusp.
   
      Parameters:
         Norm       :     fraction of photons belonging to peak
         Width_left :     the width paramater for phases left of peak; gives distance to half max
         Width_right:     same for right
         Location   :     the mode of the Lorentzian distribution
   
   """

   def init(self):
      self.p = np.asarray([1,0.1,0.2,0.1])
      self.free = np.asarray([True]*len(self.p))
      self.pnames = ['Norm','Width_left','Width_right','Location']
      self.name = 'Two-tailed Lorentzian'
      self.l1 = LCLorentzian()
      self.l2 = LCLorentzian()
      self.fwhm_scale = 2.

   def __call__(self,phases):
      norm,gamma1,gamma2,x0 = self.p
      self.__set_centers__()
      return np.where(phases < x0, self.l1(phases), self.l2(phases))

   def get_width(self,error=False,fwhm=False):
      """Override base method to return both widths."""         
      scale = self.fwhm_scale if fwhm else 1
      if error:
         return (np.asarray([self.p[1],self.p[2]])*scale, np.asarray([self.errors[1],self.errors[2]])*scale)
      return np.asarray([self.p[1],self.p[2]])*scale


   def __set_centers__(self):
      norm,gamma1,gamma2,x0 = self.p
      if gamma1 > gamma2:
         l1_x0 = x0
         l2_x0 = x0 - (gamma2*(gamma1-gamma2))**0.5
      else:
         l1_x0 = x0 + (gamma1*(gamma2-gamma1))**0.5
         l2_x0 = x0
      self.l1.p = np.asarray([norm,gamma1,l1_x0])
      self.l2.p = np.asarray([norm,gamma2,l2_x0])

   def integrate(self,x1=-np.inf,x2=np.inf):
      norm,gamma1,gamma2,x0 = self.p
      self.__set_centers__()
      return self.l1.integrate(x1,x0) + self.l2.integrate(x0,x2)
...
class LCEmpiricalFourier(LCPrimitive):
   ...
   def __call__(self,phases):
      ...
      if shift != 0:
         ...
         # It's probably a wash whether it is faster to simply subtract from the phases,
         # but it's more fun this way!
         c = np.cos(self.harmonics * shift)
         s = np.sin(self.harmonics * shift)
      ...
      return (1 +\
         2*(\
             #a * np.asarray([np.cos(ph*self.harmonics) for ph in phases]) +\
             #b * np.asarray([np.sin(ph*self.harmonics) for ph in phases])\
             a * np.asarray([np.cos(phases*k) for k in self.harmonics]).transpose() +\
             b * np.asarray([np.sin(phases*k) for k in self.harmonics]).transpose()\
           ).sum(axis=1)\
         )
