...
class TimedData(object):
    ...
    def plot_time(self, delta_max=2):
        """
        """
        df = self.df
        ta,tb = df.time.min(), df.time.max()
        t=df.time-ta
        fig,ax= plt.subplots(figsize=(15,5))
        hkw = dict(bins = np.linspace(0,tb-ta,1000), histtype='step')
        ax.hist(t, label='E>100 MeV', **hkw)
        ax.hist(t[(df.delta<delta_max) & (df.band>0)], label='delta<{}'.format(delta_max), **hkw);
        ax.set(xlabel='elapsed time from first data')
        ax.legend()
